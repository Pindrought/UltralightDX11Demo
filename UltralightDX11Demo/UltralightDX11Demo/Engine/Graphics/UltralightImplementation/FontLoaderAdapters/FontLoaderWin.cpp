//Note: I didn't edit any of this code - JMP
...
ul::String16 FontLoaderWin::fallback_font_for_characters(const ul::String16& characters, int weight, bool italic) const
{
    ComPtr<IDWriteFactory2> pDWriteFactory;

    HRESULT hr = DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory2), &pDWriteFactory);
    ...
    hr = pFontFallback->MapCharacters(&source, 0, (UINT32)characters.length(), nullptr, nullptr, toDWriteFontWeight(weight), italic ? DWRITE_FONT_STYLE_ITALIC : DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, &mappedLength, &pMappedFont, &fontScale);
    ...
    if (SUCCEEDED(hr) && !exists) // If we didn't find a match, try again with US English
    ...
    return ul::String16((const ul::Char16*)name.get(), nameLength);
}

static ul::RefPtr<ul::FontFile> LoadFont(const ul::String16& family, int weight, bool italic)
{
    ...
    if (FAILED(hr) || pFontFamily->GetFontCount() == 0)
        return nullptr;

    // Get the first matching font
    ComPtr<IDWriteFont> pFont;
    hr = pFontFamily->GetFirstMatchingFont(toDWriteFontWeight(weight), DWRITE_FONT_STRETCH_NORMAL, italic ? DWRITE_FONT_STYLE_ITALIC : DWRITE_FONT_STYLE_NORMAL, &pFont);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        ul::String16 pathStr(path, pathLength);
        return ul::FontFile::Create(pathStr);
    }
    ...
    ul::Ref<ul::Buffer> result = ul::Buffer::Create(fragmentStart, (size_t)fileSize);

    pFontFileStream->ReleaseFileFragment(context);

    return ul::FontFile::Create(result);
}

ul::RefPtr<ul::FontFile> FontLoaderWin::Load(const ul::String16& family, int weight, bool italic)
{
